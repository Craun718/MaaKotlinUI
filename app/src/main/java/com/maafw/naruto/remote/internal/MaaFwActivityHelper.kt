...
object MaaFwActivityHelper {
    ...
    private val setLaunchWindowingMode by lazy {
        ...
        }.onFailure { Ln.w("MaaFwActivityHelper: setLaunchWindowingMode reflection failed: ${it.message}") }.getOrNull()
    }
    ...
    fun startActivity(intent: Intent, displayId: Int = 0, forceFullscreen: Boolean = false): Boolean {
        val am = ServiceManager.getActivityManager() ?: run {
            Ln.e("MaaFwActivityHelper.startActivity: ActivityManager is null")
            return false
        }
        return try {
            ...
            if (displayId != Display.DEFAULT_DISPLAY) {
                ...
                if (forceFullscreenOnVirtualDisplay || forceFullscreen) {
                    runCatching {
                        ...
                        Ln.e("MaaFwActivityHelper.startActivity: invoke setLaunchWindowingMode failed: ${it.message}")
                    }
                }
            }
            val ret = try {
                ...
                Ln.w("MaaFwActivityHelper.startActivity: am.startActivity failed: ${e.message}")
                -1
            }
            if (ret < 0) {
                Ln.w("MaaFwActivityHelper.startActivity: returned $ret, fallback to am command")
                startViaAmCommand(intent, displayId)
            } else {
                true
            }
        } catch (e: Exception) {
            Ln.w("MaaFwActivityHelper.startActivity: failed, fallback to am command: ${e.message}")
            startViaAmCommand(intent, displayId)
        }
    }
    ...
    fun startApp(
        ...
        val pm = ShellContext.get().packageManager
        ...
        if (intent == null) {
            Ln.w("MaaFwActivityHelper.startApp: cannot create launch intent for $packageName")
            return false
        }
        ...
        Ln.i("MaaFwActivityHelper.startApp: ${intent.component?.flattenToShortString()}")
        ...
        while (true) {
            when (val current = getAppDisplayId(packageName)) {
                ...
                DISPLAY_NO_TASK -> Unit // 任务尚未出现，继续等待
                    ...
                    Ln.w("MaaFwActivityHelper.ensureAppOnDisplay: $packageName drifted to display $current (expected $displayId), trying to move it back")
                    return repinAppToDisplay(packageName, displayId)
                }
            }
            if (SystemClock.uptimeMillis() >= deadline) {
                Ln.w("MaaFwActivityHelper.ensureAppOnDisplay: no task of $packageName observed within ${timeoutMs}ms, passing leniently")
                return true
            }
    ...
    fun repinAppToDisplay(packageName: String, displayId: Int): Boolean {
        if (moveAppTaskToDisplay(packageName, displayId)) {
            ...
            if (isAppOnDisplay(packageName, displayId)) {
                Ln.i("MaaFwActivityHelper.repinAppToDisplay: $packageName moved back to display $displayId")
                return true
            }
        }
        val pm = ShellContext.get().packageManager
        ...
        if (intent != null) {
            ...
            if (startActivity(intent, displayId, forceFullscreen = true)) {
                SystemClock.sleep(REPIN_SETTLE_MS)
                if (isAppOnDisplay(packageName, displayId)) {
                    Ln.i("MaaFwActivityHelper.repinAppToDisplay: $packageName relaunched onto display $displayId")
                    return true
                }
            }
        }
        Ln.e("MaaFwActivityHelper.repinAppToDisplay: failed to pin $packageName on display $displayId")
        return false
    }
    ...
    private fun findRecentTask(packageName: String): ActivityManager.RunningTaskInfo? {
        val am = ShellContext.get().getSystemService(Context.ACTIVITY_SERVICE) as? ActivityManager ?: return null
    ...
    fun moveAppTaskToDisplay(packageName: String, displayId: Int): Boolean {
        val task = runCatching { findRecentTask(packageName) }.getOrNull() ?: run {
            Ln.w("MaaFwActivityHelper.moveAppTaskToDisplay: no running task of $packageName")
            return false
        }
        @Suppress("DEPRECATION")
        ...
        if (method != null) {
            runCatching {
                ...
                Ln.i("MaaFwActivityHelper.moveAppTaskToDisplay: ${method.name}($taskId, $displayId) invoked")
                return true
            }.onFailure {
                Ln.w("MaaFwActivityHelper.moveAppTaskToDisplay: ${method.name} failed: ${it.message}")
            }
        }
    ...
    private val activityTaskManager: Any? by lazy {
        runCatching {
            ...
            Ln.w("MaaFwActivityHelper.activityTaskManager: reflection failed: ${it.message}")
        }.getOrNull()
    }
    ...
    private val moveTaskToDisplayMethod: Method? by lazy {
        ...
        Ln.w("MaaFwActivityHelper.moveTaskToDisplayMethod: no candidate method found")
        null
    }
    ...
    private fun moveTaskViaAmCommand(taskId: Int, displayId: Int): Boolean {
        return try {
            ...
            Ln.i("MaaFwActivityHelper.moveTaskViaAmCommand: exec: ${args.joinToString(" ")}")
            val process = Runtime.getRuntime().exec(args)
            val exitCode = process.waitFor()
            val stderr = process.errorStream.bufferedReader().use { it.readText() }.trim()
            if (stderr.isNotEmpty()) Ln.w("MaaFwActivityHelper.moveTaskViaAmCommand: stderr: $stderr")
            if (exitCode != 0) {
                Ln.w("MaaFwActivityHelper.moveTaskViaAmCommand: exitCode=$exitCode")
                return false
            }
            true
        } catch (e: Exception) {
            Ln.e("MaaFwActivityHelper.moveTaskViaAmCommand: failed: ${e.message}")
            false
        }
    }
    ...
    private val taskDisplayIdField: Field? by lazy {
        runCatching {
            ...
            Ln.w("MaaFwActivityHelper.taskDisplayIdField: reflection failed: ${it.message}")
        }.getOrNull()
    }
    ...
    private fun startViaAmCommand(intent: Intent, displayId: Int): Boolean {
        return try {
            ...
            Ln.i("MaaFwActivityHelper.startViaAmCommand: exec: ${args.joinToString(" ")}")
            val process = Runtime.getRuntime().exec(args)
            val exitCode = process.waitFor()
            // am start 输出量极小（远小于管道缓冲），先 waitFor 再读不会死锁
            val stdout = process.inputStream.bufferedReader().use { it.readText() }.trim()
            val stderr = process.errorStream.bufferedReader().use { it.readText() }.trim()
            if (stdout.isNotEmpty()) Ln.i("MaaFwActivityHelper.startViaAmCommand: stdout: $stdout")
            if (stderr.isNotEmpty()) Ln.w("MaaFwActivityHelper.startViaAmCommand: stderr: $stderr")
            if (exitCode != 0) {
                Ln.w("MaaFwActivityHelper.startViaAmCommand: exitCode=$exitCode")
                return false
            }
            Ln.i("MaaFwActivityHelper.startViaAmCommand: success")
            true
        } catch (e: Exception) {
            Ln.e("MaaFwActivityHelper.startViaAmCommand: failed: ${e.message}")
            false
        }
    }

    /**
     * 用 am start --display 把应用直接启动到指定虚拟屏（ ）。
    ...
    fun startAppViaAmCommand(packageName: String, displayId: Int): Boolean {
        ...
        return try {
            ...
            Ln.i("MaaFwActivityHelper.startAppViaAmCommand: exec: ${args.joinToString(" ")}")
            val process = Runtime.getRuntime().exec(args)
            val exitCode = process.waitFor()
            val stdout = process.inputStream.bufferedReader().use { it.readText() }.trim()
            val stderr = process.errorStream.bufferedReader().use { it.readText() }.trim()
            if (stdout.isNotEmpty()) Ln.i("MaaFwActivityHelper.startAppViaAmCommand: stdout: $stdout")
            if (stderr.isNotEmpty()) Ln.w("MaaFwActivityHelper.startAppViaAmCommand: stderr: $stderr")
            if (exitCode != 0) {
                Ln.w("MaaFwActivityHelper.startAppViaAmCommand: exitCode=$exitCode")
                return false
            }
            Ln.i("MaaFwActivityHelper.startAppViaAmCommand: success")
            true
        } catch (e: Exception) {
            Ln.e("MaaFwActivityHelper.startAppViaAmCommand: failed: ${e.message}")
            false
        }
    }
}