...
object MaaFwVirtualDisplay {
    ...
    fun setMonitorSurface(surface: Surface?) {
        // 释放旧 surface 防止句柄泄漏（切屏/退出全屏时反复绑定）
    ...
    private fun startInternal(): Int {
        val cfg = config.get()
        Ln.i("MaaFwVirtualDisplay startInternal ${cfg.width}x${cfg.height}@${cfg.dpi}")
        return try {
            Ln.i("[VD-STEP] setupNativeCapturer(${cfg.width}, ${cfg.height})...")
            val surface = com.maafw.naruto.bridge.BridgeNativeLib.setupNativeCapturer(cfg.width, cfg.height)
            ...
            Ln.i("MaaFwVirtualDisplay started, displayId=$id")
            id
        } catch (e: Throwable) {
            Ln.e("MaaFwVirtualDisplay start failed", e)
    ...
    private fun releaseResources() {
        ...
        com.maafw.naruto.bridge.BridgeNativeLib.releaseNativeCapturer()
        displayId.set(-1)
    }
    ...
    private fun createVirtualDisplay(surface: Surface, cfg: DisplayConfig, fullFlags: Boolean = true) {
        ...
        try {
            ...
            // E1 旋转处理（ROM 兼容）：虚拟屏 rotation !=0 -> freezeRotation(0)；
            // 物理屏自然方向 0 而虚拟屏有旋转（横屏原生设备/掌机定制 ROM）-> setForcedDisplaySize 强制横屏上报
            runCatching {
                val rotation = vd.display.rotation
                if (rotation != android.view.Surface.ROTATION_0) {
                    wm.freezeRotation(vdId, android.view.Surface.ROTATION_0)
                    Ln.i("VirtualDisplay rotation=$rotation -> freezeRotation(0)")
                    if (physicalRotation == android.view.Surface.ROTATION_0) {
                        wm.setForcedDisplaySize(vdId, cfg.width, cfg.height)
                        Ln.i("VirtualDisplay setForcedDisplaySize ${cfg.width}x${cfg.height}（横屏原生设备）")
                    }
                } else {
                    Ln.i("VirtualDisplay rotation=0（无需旋转修正）")
                }
            }.onFailure { Ln.w("VirtualDisplay rotation 处理失败: ${it.message}") }

            // 新虚拟屏默认处于熄灭状态（Android 14+ 必须显式 requestDisplayPower 点亮）。
            // 不点亮的话 SurfaceFlinger 不渲染该 display：截图/预览黑屏，
            // MaaFramework screencap 拿不到帧，管线会卡死在 start_up，StartApp 永不执行。
            runCatching {
